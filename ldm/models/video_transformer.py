...
class SpaceTimeTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 num_frames=8, time_init='rand', attention_style='frozen-in-time', verbose=False):
        """
        Args:
    ...
    def forward_features(self, x, return_intermediate=False, max_pool_patches=False, mean_pool_patches=False):
        ...
        if return_intermediate:
            x = self.norm(x)
            if max_pool_patches:
                # max pool patches for each frame but keep the same number of frames
                x = x[:, 1:].reshape(b, self.num_frames, -1, self.embed_dim)
                x = x.max(dim=2)[0]
            if mean_pool_patches:
                # mean pool patches for each frame but keep the same number of frames
                x = x[:, 1:].reshape(b, self.num_frames, -1, self.embed_dim)
                x = x.mean(dim=2)
        else:
            x = self.norm(x)[:, 0]
    ...
    def forward(self, x, return_intermediate=False, max_pool_patches=False, mean_pool_patches=False):
        x = self.forward_features(x, return_intermediate, max_pool_patches, mean_pool_patches)
        x = self.head(x)
        return x

if __name__ == "__main__":
    # network = SpaceTimeTransformer(num_frames=4)
    # data = torch.rand((3, 4, 3, 224, 224))
    network = SpaceTimeTransformer(num_frames=16, time_init='zeros')
    network.head = nn.Identity()
    network.pre_logits = nn.Identity()
    data = torch.rand((3, 16, 3, 224, 224))
    network(data, return_intermediate=True, max_pool_patches=True, mean_pool_patches=False)