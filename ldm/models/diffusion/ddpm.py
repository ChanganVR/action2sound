...
from ldm.models.autoencoder import Sound_AutoencoderKL
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        # whether determinsitc:
        self.encode_deterministic = deterministic
    ...
    def on_train_batch_start(self, batch, batch_idx, dataloader_idx):
        ...
        if self.scale_by_std and self.current_epoch == 0 and self.global_step == 0 and batch_idx == 0 and not self.restarted_from_ckpt:
            ...
            z = self.get_first_stage_encoding(encoder_posterior, self.encode_deterministic).detach()
    ...
    def get_first_stage_encoding(self, encoder_posterior, deterministic=False):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        x = x.to(self.device)
        encoder_posterior = self.encode_first_stage(x)
        z = self.get_first_stage_encoding(encoder_posterior, self.encode_deterministic).detach()       # B x 128 x 3750
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ['caption', 'coordinates_bbox']:
                    xc = batch[cond_key]                                    # B x L x 2048
    ...
    def log_sound(self, batch, N=4, n_row=4, sample=True, ddim_steps=250, ddim_eta=0.0, return_keys=None,
        ...
        if sample:
            with self.ema_scope("Plotting"):
                samples, intermediates = self.sample_log(cond=c, batch_size=N, ddim=use_ddim, ddim_steps=ddim_steps, eta=ddim_eta)
            x_samples = self.decode_first_stage(samples)
            # clip:
