        ...
        if guidance_type == "uncond":
            ...
            if guidance_scale == 1. or unconditional_condition is None:
                ...
                if type(condition) == list:
                    assert type(unconditional_condition) == list
                    x2 = x.clone()
                    t2 = t_continuous.clone()
                    noise_uncond = noise_pred_fn(x, t_continuous, cond=unconditional_condition)
                    noise = noise_pred_fn(x2, t2, cond=condition)
                else:
                    x_in = torch.cat([x] * 2)
...
# Diff_Foley Revise:
def model_wrapper_with_classifier(
    model,
    noise_schedule,
    model_type="noise",
    model_kwargs={},
    ...
    def cond_grad_fn_classifier(x, t_input, origin_cond):
        """
        Compute the gradient of the classifier, i.e. nabla_{x} log p_t(cond | x_t).
        """
        with torch.enable_grad():
            x_in = x.detach().requires_grad_(True)
            prob_logits = classifier(x_in, t=t_input, video_feat=origin_cond)
            log_probs = torch.log(prob_logits)
            # log_prob = classifier_fn(x_in, t_input, condition, **classifier_kwargs)
            return torch.autograd.grad(log_probs.sum(), x_in)[0]
    ...
    def model_fn(x, t_continuous):
        ...
        elif guidance_type == "double-guide":
            if guidance_scale == 1. or unconditional_condition is None:
                return noise_pred_fn(x, t_continuous, cond=condition)
            else:
                # CFG:
                ...
                # CG:
                t_input = get_model_input_time(t_continuous)
                cond_grad = cond_grad_fn_classifier(x, t_input, origin_cond)
                sigma_t = noise_schedule.marginal_std(t_continuous)

                noise = noise_uncond + guidance_scale * (noise - noise_uncond)
                noise = noise - classifier_guide_scale * expand_dims(sigma_t, dims=cond_grad.dim()) * cond_grad
                return noise

    assert model_type in ["noise", "x_start", "v"]
    assert guidance_type in ["uncond", "classifier", "classifier-free", "double-guide"]
    return model_fn